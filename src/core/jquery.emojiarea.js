 ...
 */
import { ConfigStorage } from './emoji/lib/js/emoji.util.js';
import Tether from './emoji/lib/js/tether.min.js';
...
(function($, window, document) {
	...
	$.fn.emojiarea = function(options, emojiConfig) {
		options = $.extend({}, options);
        globalEmojiConfig = emojiConfig;
		return this
			.each(function() {
    ...
    self.emojiMenu = new EmojiMenu(self);
		...
		this.setupButton(self.$button);
	};

	EmojiArea.prototype.setupButton = function(button) {
    	var self = this;
	...
	EmojiArea.createIcon = function(emoji, menu) {
		...
		var column = emoji[2];
		var name = emoji[3];
		var filename = $.emojiarea.assetsPaths['spritesheet_'+category]
        var blankGifPath = $.emojiarea.assetsPaths.blankUrl;
		var iconSize = menu && false ? 26 : $.emojiarea.iconSize
		var xoffset = -(iconSize * column);
		var yoffset = -(iconSize * row);
		var scaledWidth = (globalEmojiConfig.emojiCategorySpritesheetDimens[category][1] * iconSize);
		var scaledHeight = (globalEmojiConfig.emojiCategorySpritesheetDimens[category][0] * iconSize);
    ...
    this.$button = $textarea.after("<i class='emoji-picker-icon emoji-picker " + this.options.popupButtonClasses + "' data-id='" + id + "' data-type='picker'></i>").next();
...
//      scrollHeight -= parseInt(this.css('padding-top')) + parseInt(this.css('padding-bottom'));
    ...
    self.id = emojiarea.id;
        ...
        this.$menu.css({'background-color':'white',
                        'overflow-y': 'auto',
                        'max-height': '184px'})
...
/*		 if (!false) { this.$itemsWrap.nanoScroller({ scroll: 'top'
		 }); }*/
